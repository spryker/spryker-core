...
namespace Unit\SprykerFeature\Zed\Client\Communication\Plugin;
...
use SprykerFeature\Zed\Client\Communication\Plugin\ClientControllerListenerPlugin;
use Symfony\Component\HttpFoundation\JsonResponse;
use Unit\SprykerFeature\Zed\Client\Communication\Plugin\Fixture\FooTransfer;
use Unit\SprykerFeature\Zed\Client\Communication\Plugin\Fixture\NotClientController;
use Unit\SprykerFeature\Zed\Client\Communication\Plugin\Fixture\Request;
use Unit\SprykerFeature\Zed\Client\Communication\Plugin\Fixture\ClientController;
use Unit\SprykerFeature\Zed\Client\Communication\Plugin\Fixture\TransferServer;

/**
 * @group Zed
 * @group Communication
 * @group Client
 * @group ClientListener
 */
class ClientControllerListenerPluginTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testIsClientController()
    {
        $eventMock = new Fixture\FilterControllerEvent();
        $ClientController = new ClientController();
        $action = 'fooAction';
        $eventMock->setController([$ClientController, $action]);

        $ClientControllerListenerPlugin = new ClientControllerListenerPlugin(
            $this->createFactoryMock(),
            $this->createLocatorMock()
        );
        $ClientControllerListenerPlugin->onKernelController($eventMock);
    ...
    public function testIsNotClientController()
    {
        $action = 'fooAction';
        $eventMock = new Fixture\FilterControllerEvent();
        $ClientController = new NotClientController();
        $eventMock->setController([$ClientController, $action]);
