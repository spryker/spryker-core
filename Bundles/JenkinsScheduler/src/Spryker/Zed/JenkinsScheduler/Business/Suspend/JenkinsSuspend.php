...
class JenkinsSuspend implements JenkinsSuspendInterface
{
    ...
    public function suspendJenkinsScheduler(string $schedulerId, SchedulerTransfer $schedulerTransfer, SchedulerResponseTransfer $schedulerResponseTransfer): SchedulerResponseTransfer
    {
        ...
        if (!empty($schedulerJobNames)) {
            return $this->jenkinsJobStatusUpdater->updateJenkinsJobStatusByJobsName($schedulerId, $schedulerTransfer, $schedulerResponseTransfer, static::SUSPEND_JOB_URL_TEMPLATE);
        }

        return $this->jenkinsJobStatusUpdater->updateAllJenkinsJobsStatus($schedulerId, $schedulerTransfer, $schedulerResponseTransfer, static::SUSPEND_JOB_URL_TEMPLATE);
    }
}
