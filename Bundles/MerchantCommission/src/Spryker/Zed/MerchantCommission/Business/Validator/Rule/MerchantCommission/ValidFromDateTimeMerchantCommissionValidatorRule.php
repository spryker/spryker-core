...
class ValidFromDateTimeMerchantCommissionValidatorRule implements MerchantCommissionValidatorRuleInterface, TerminationAwareValidatorRuleInterface
{
    /**
     * @var string
     */
    ...
    public function validate(ArrayObject $merchantCommissionTransfers): ErrorCollectionTransfer
    {
        ...
        foreach ($merchantCommissionTransfers as $entityIdentifier => $merchantCommissionTransfer) {
            if ($merchantCommissionTransfer->getValidFrom() === null) {
                continue;
            }

            if ($this->isValidDateTime($merchantCommissionTransfer->getValidFromOrFail())) {
                continue;
            }
