...
class MerchantProductSearchWritePublisherPluginTest extends Unit
{
    ...
    public function testMerchantProductSearchWritePublisherPluginStoresData(): void
    {
        ...
        $idProductAbstractMerchant = $this->tester->addMerchantProductRelation($merchantTransfer->getIdMerchant(), $productConcreteTransfer->getFkProductAbstract());
        ...
        $eventTransfers = [
            (new EventEntityTransfer())->setId($idProductAbstractMerchant),
        ];

        // Act
        $merchantProductSearchWritePublisher->handleBulk($eventTransfers, MerchantProductSearchConfig::MERCHANT_PRODUCT_ABSTRACT_PUBLISH);
