...
class ValidUniqueStoreCurrencyGrossNetPriceDataConstraintValidator extends AbstractConstraintValidator
{
    ...
    public function validate($value, Constraint $constraint): void
    {
        ...
        $priceProductFilterTransfer->setCurrency($moneyValueTransfer->getCurrency())
            ->setStoreName($moneyValueTransfer->getStoreOrFail()->getName())
            ->setPriceDimension($value->getPriceDimension());

        if ($constraint->getReader()->findPriceFor($priceProductFilterTransfer)) {
