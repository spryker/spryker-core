...
class PriceProductMapper implements PriceProductMapperInterface
{
    ...
    public function mapPricesToPriceProductTransfers(array $prices): array
    {
        ...
        foreach ($prices as $price) {
            $productPriceCollection[] = $this->mapPriceToTransfer(
                $price
            );
        }
        return $productPriceCollection;
     ...
     * @return \Generated\Shared\Transfer\PriceProductTransfer
     */
    protected function mapPriceToTransfer(array $price): PriceProductTransfer
    {
        $priceTypeTransfer = (new PriceTypeTransfer())
            ->setName('DEFAULT')// TODO
            ->setPriceModeConfiguration('GROSS_MODE'); // TODO
        $currencyTransfer = $this->currencyFacade
            ->getByIdCurrency($price['fk_currency']);
        $moneyValueTransfer = (new MoneyValueTransfer())
            ->fromArray($price, true)
            ->setIdEntity($price['id_price_product_store'])
            ->setNetAmount($price['net_price'])
            ->setGrossAmount($price['gross_price'])
            ->setCurrency($currencyTransfer);
        $priceProductDimensionTransfer = new PriceProductDimensionTransfer();
        $priceProductDimensionTransfer->fromArray($price, true);
        return (new PriceProductTransfer())
            ->fromArray($price, true)
            ->setSkuProduct($price['PriceProduct']['Product']['sku'] ?? $price['product_sku'])
            ->setIdProduct($price['PriceProduct']['fk_product'])
            ->setIdProductAbstract($price['PriceProduct']['fk_product_abstract'])
            ->setPriceType($priceTypeTransfer)
            ->setPriceTypeName($priceTypeTransfer->getName())
            ->setMoneyValue($moneyValueTransfer)
            ->setPriceDimension($priceProductDimensionTransfer)
