...
class PriceProductConcreteStorageReader implements PriceProductConcreteStorageReaderInterface
{
    ...
    public function findPriceProductConcreteStorageTransfer(string $sku): ?PriceProductStorageTransfer
    {
        ...
        $key = $this->synchronizationService
            ->getStorageKeyBuilder(PriceProductStorageConstants::PRICE_CONCRETE_RESOURCE_NAME)
        ...
        $priceProductConcreteStorageData = $this->storageClient->get($mappingResource['key']);
        if ($priceProductConcreteStorageData === null) {
            return null;
        }

        return (new PriceProductStorageTransfer)->fromArray($priceProductConcreteStorageData, true);
    }
}
