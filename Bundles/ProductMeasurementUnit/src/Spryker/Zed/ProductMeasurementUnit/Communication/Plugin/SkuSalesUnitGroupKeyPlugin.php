 ...
 */
class SkuSalesUnitGroupKeyPlugin extends AbstractPlugin implements ItemExpanderPluginInterface
{
    /**
    ...
    {
        if (!$cartItem->getQuantitySalesUnit()) {
            return $cartItem->getGroupKey();
        }

        return $cartItem->getGroupKey() . '_id_sales_unit_' . $cartItem->getQuantitySalesUnit()->getIdProductMeasurementSalesUnit();
    }
}
