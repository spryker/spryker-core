 ...
 * @method     ChildSpyProductOptionValuePriceQuery orderByPrice($order = Criteria::ASC) Order by the price column
 *
 ...
 * @method     ChildSpyProductOptionValuePriceQuery groupByPrice() Group by the price column
 *
 ...
 *
 * @method     ChildSpyProductOptionValuePriceQuery leftJoinSpyProductOptionValue($relationAlias = null) Adds a LEFT JOIN clause to the query using the SpyProductOptionValue relation
 * @method     ChildSpyProductOptionValuePriceQuery rightJoinSpyProductOptionValue($relationAlias = null) Adds a RIGHT JOIN clause to the query using the SpyProductOptionValue relation
 * @method     ChildSpyProductOptionValuePriceQuery innerJoinSpyProductOptionValue($relationAlias = null) Adds a INNER JOIN clause to the query using the SpyProductOptionValue relation
 *
 * @method     \SprykerFeature\Zed\ProductOption\Persistence\Propel\SpyProductOptionValueQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 * @method     ChildSpyProductOptionValuePrice findOne(ConnectionInterface $con = null) Return the first ChildSpyProductOptionValuePrice matching the query
 ...
 * @method     ChildSpyProductOptionValuePrice[]|ObjectCollection findByPrice(int $price) Return ChildSpyProductOptionValuePrice objects filtered by the price column
...
abstract class SpyProductOptionValuePriceQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'zed', $modelName = '\\SprykerFeature\\Zed\\ProductOption\\Persistence\\Propel\\SpyProductOptionValuePrice', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = SpyProductOptionValuePriceTableMap::getInstanceFromPool((string) $key))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT id_product_option_value_price, price FROM spy_product_option_value_price WHERE id_product_option_value_price = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            SpyProductOptionValuePriceTableMap::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * $query->filterByIdProductOptionValuePrice(array(12, 34)); // WHERE id_product_option_value_price IN (12, 34)
     * $query->filterByIdProductOptionValuePrice(array('min' => 12)); // WHERE id_product_option_value_price > 12
     * </code>
     *
     ...
     * @return $this|ChildSpyProductOptionValuePriceQuery The current query, for fluid interface
     */
    public function filterByIdProductOptionValuePrice($idProductOptionValuePrice = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(SpyProductOptionValuePriceTableMap::COL_ID_PRODUCT_OPTION_VALUE_PRICE, $idProductOptionValuePrice, $comparison);
    }
    ...
    public function filterByPrice($price = null, $comparison = null)
    {
        if (is_array($price)) {
            $useMinMax = false;
            if (isset($price['min'])) {
                $this->addUsingAlias(SpyProductOptionValuePriceTableMap::COL_PRICE, $price['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($price['max'])) {
                $this->addUsingAlias(SpyProductOptionValuePriceTableMap::COL_PRICE, $price['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(SpyProductOptionValuePriceTableMap::COL_PRICE, $price, $comparison);
    }
    ...
    public function filterBySpyProductOptionValue($spyProductOptionValue, $comparison = null)
    {
        if ($spyProductOptionValue instanceof \SprykerFeature\Zed\ProductOption\Persistence\Propel\SpyProductOptionValue) {
            return $this
                ->addUsingAlias(SpyProductOptionValuePriceTableMap::COL_ID_PRODUCT_OPTION_VALUE_PRICE, $spyProductOptionValue->getFkProductOptionValuePrice(), $comparison);
     ...
     * @return $this|ChildSpyProductOptionValuePriceQuery The current query, for fluid interface
     */
    public function joinSpyProductOptionValue($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
     ...
     */
    public function useSpyProductOptionValueQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
