...
";
    ...
    protected function addToArrayKeyLookUp(?string $phpName, Table $table, bool $plural): string
    {
        ...
        if ($phpName) {
            ...
            // 1. `(?<!^)[A-Z]`: Matches any uppercase letter that is not at the start of the string.
            // 2. `(?<=[a-z])[0-9]`: Matches any number that is preceded by a lowercase letter.
            // The `|` means OR, so it finds either of those cases.
            // It replaces the found match with an underscore and the match itself `_$0`.
            $snakeCase = (string)preg_replace('/(?<!^)[A-Z]|(?<=[a-z])[0-9]+/', '_$0', $phpName);

            $camelCaseName = lcfirst($phpName);
            $fieldName = strtolower($snakeCase);
        }
