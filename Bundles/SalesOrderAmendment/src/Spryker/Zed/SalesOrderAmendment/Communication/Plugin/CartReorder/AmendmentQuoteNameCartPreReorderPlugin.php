 ...
 */
class AmendmentQuoteNameCartPreReorderPlugin extends AbstractPlugin implements CartPreReorderPluginInterface
{
    /**
     * {@inheritDoc}
     ...
     * - Updates `CartReorderTransfer.quote.name` with custom amendment quote name.
     *
     * @api
     *
    ...
    public function preReorder(
        ...
        if ($cartReorderRequestTransfer->getIsAmendment()) {
            $cartReorderTransfer->getQuoteOrFail()->setName(
                sprintf($this->getConfig()->getCartReorderQuoteNameFormat(), $cartReorderRequestTransfer->getOrderReferenceOrFail()),
            );
        }
