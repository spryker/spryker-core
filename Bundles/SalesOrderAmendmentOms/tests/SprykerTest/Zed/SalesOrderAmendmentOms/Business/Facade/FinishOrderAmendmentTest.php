...
class FinishOrderAmendmentTest extends Unit
{
    ...
    public function testShouldFinishOrderAmendmentWhenAmendableOrderReferenceIsProvided(): void
    {
        ...
        $this->tester->setItemState($amendableSaveOrderTransfer->getOrderItems()->offsetGet(0)->getIdSalesOrderItemOrFail(), static::ORDER_ITEM_STATE_ORDER_AMENDMENT_LOCK);
        $this->tester->setItemState($amendableSaveOrderTransfer->getOrderItems()->offsetGet(1)->getIdSalesOrderItemOrFail(), static::ORDER_ITEM_STATE_ORDER_AMENDMENT_LOCK);
