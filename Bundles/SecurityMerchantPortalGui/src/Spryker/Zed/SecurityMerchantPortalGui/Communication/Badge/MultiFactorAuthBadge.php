...
class MultiFactorAuthBadge implements BadgeInterface
{
    ...
    /**
     * @param array<\Spryker\Zed\SecurityMerchantPortalGuiExtension\Dependency\Plugin\AuthenticationHandlerPluginInterface> $merchantPortalUserMultiFactorAuthenticationHandlerPlugins
     */
    public function __construct(
        protected array $merchantPortalUserMultiFactorAuthenticationHandlerPlugins
    ) {
    }
    ...
    {
        foreach ($this->merchantPortalUserMultiFactorAuthenticationHandlerPlugins as $plugin) {
            if ($plugin->isApplicable(static::MERCHANT_PORTAL_USER_MULTI_FACTOR_AUTHENTICATION_HANDLER_NAME) === false) {
                continue;
            }
