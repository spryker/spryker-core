...
class KeyLengthServiceValidatorRule implements ServiceValidatorRuleInterface, TerminationAwareValidatorRuleInterface
{
    /**
     * @var int
     */
    protected const SERVICE_KEY_MIN_LENGTH = 1;

    /**
     * @var int
     */
    protected const SERVICE_KEY_MAX_LENGTH = 255;
    ...
    public function validate(ArrayObject $serviceTransfers): ErrorCollectionTransfer
    {
        ...
        foreach ($serviceTransfers as $entityIdentifier => $serviceTransfer) {
            if (!$this->isServiceKeyLengthValid($serviceTransfer->getKeyOrFail())) {
                $this->errorAdder->addError(
                    $errorCollectionTransfer,
                    $entityIdentifier,
                    static::GLOSSARY_KEY_VALIDATION_SERVICE_KEY_WRONG_LENGTH,
                    [
                        static::GLOSSARY_KEY_PARAMETER_MIN => static::SERVICE_KEY_MIN_LENGTH,
                        static::GLOSSARY_KEY_PARAMETER_MAX => static::SERVICE_KEY_MAX_LENGTH,
                    ],
                );
            }
        }
     ...
     * @return bool
     */
    protected function isServiceKeyLengthValid(string $key): bool
    {
        return mb_strlen($key) >= static::SERVICE_KEY_MIN_LENGTH
            && mb_strlen($key) <= static::SERVICE_KEY_MAX_LENGTH;
    }
}
