...
class ServicePointServiceTypeExistenceServicePointServiceValidatorRule implements ServicePointServiceValidatorRuleInterface
{
    /**
     * @var string
     */
    ...
    public function validate(ArrayObject $servicePointServiceTransfers): ErrorCollectionTransfer
    {
        ...
        foreach ($servicePointServiceTransfers as $entityIdentifier => $servicePointServiceTransfer) {
            if ($this->hasServicePointToServiceTypeRelation($servicePointServiceTransfer)) {
                $this->errorAdder->addError(
                    $errorCollectionTransfer,
                    $entityIdentifier,
                    static::GLOSSARY_KEY_VALIDATION_SERVICE_POINT_SERVICE_TYPE_RELATION_ALREADY_EXISTS,
                );
            }
        }
