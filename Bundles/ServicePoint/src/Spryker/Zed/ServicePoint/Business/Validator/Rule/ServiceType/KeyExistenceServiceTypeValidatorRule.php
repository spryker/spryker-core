...
class KeyExistenceServiceTypeValidatorRule implements ServiceTypeValidatorRuleInterface
{
    /**
     * @var string
     */
    ...
    public function validate(ArrayObject $serviceTypeTransfers): ErrorCollectionTransfer
    {
        ...
        foreach ($serviceTypeTransfers as $entityIdentifier => $serviceTypeTransfer) {
            if ($this->hasServiceTypeWithKey($serviceTypeTransfer)) {
     ...
     * @return bool
     */
    protected function hasServiceTypeWithKey(ServiceTypeTransfer $serviceTypeTransfer): bool
    {
        $serviceTypeConditionsTransfer = (new ServiceTypeConditionsTransfer())->addKey($serviceTypeTransfer->getKeyOrFail());
