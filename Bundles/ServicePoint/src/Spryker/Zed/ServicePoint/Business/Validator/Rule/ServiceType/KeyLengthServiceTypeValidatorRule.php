...
class KeyLengthServiceTypeValidatorRule implements ServiceTypeValidatorRuleInterface, TerminationAwareValidatorRuleInterface
{
    /**
     * @var int
     */
    protected const SERVICE_TYPE_KEY_MIN_LENGTH = 1;

    /**
     * @var int
     */
    protected const SERVICE_TYPE_KEY_MAX_LENGTH = 255;
    ...
    public function validate(ArrayObject $serviceTypeTransfers): ErrorCollectionTransfer
    {
        ...
        foreach ($serviceTypeTransfers as $entityIdentifier => $serviceTypeTransfer) {
            if (!$this->isKeyLengthValid($serviceTypeTransfer->getKeyOrFail())) {
                $this->errorAdder->addError(
                    $errorCollectionTransfer,
                    $entityIdentifier,
                    static::GLOSSARY_KEY_VALIDATION_SERVICE_TYPE_KEY_WRONG_LENGTH,
                    [
                        static::GLOSSARY_KEY_PARAMETER_MIN => static::SERVICE_TYPE_KEY_MIN_LENGTH,
                        static::GLOSSARY_KEY_PARAMETER_MAX => static::SERVICE_TYPE_KEY_MAX_LENGTH,
                    ],
                );
            }
        }
    ...
    protected function isKeyLengthValid(string $key): bool
    {
        return mb_strlen($key) >= static::SERVICE_TYPE_KEY_MIN_LENGTH
            && mb_strlen($key) <= static::SERVICE_TYPE_KEY_MAX_LENGTH;
    }
}
