...
class ContextNotFoundRule implements StoreContextValidatorRuleInterface
{
    /**
     * @var string
     */
    protected const ERROR_MESSAGE_STORE_CONTEXT_NOT_FOUND = 'Store context not found for store id: %id%.';
    ...
    public function validateStoreContext(StoreContextTransfer $storeContextTransfer): array
    {
        ...
        if ($storeContextCollectionTransfer->getApplicationContexts()->count() === 0) {
            return [
                (new ErrorTransfer())
                    ->setMessage(static::ERROR_MESSAGE_STORE_CONTEXT_NOT_FOUND)
                    ->setParameters([
