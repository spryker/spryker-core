...
class WebProfilerApplicationPlugin extends AbstractPlugin implements ApplicationPluginInterface, BootableApplicationPluginInterface
{
    ...
    protected function getRouteDefinitions(ContainerInterface $container): array
    {
        $profilerController = function () use ($container) {
            return new ProfilerController($container->get(static::SERVICE_ROUTER), $container->get(static::SERVICE_PROFILER), $container->get(static::SERVICE_TWIG), $this->getDataCollectorPluginTemplates());
        };

        $routerController = function () use ($container) {
            return new RouterController($container->get(static::SERVICE_PROFILER), $container->get(static::SERVICE_TWIG), $container->get(static::SERVICE_ROUTER));
        };

        $exceptionController = function () use ($container) {
            return new ExceptionController($container->get(static::SERVICE_PROFILER), $container->get(static::SERVICE_TWIG), $container->get('debug'));
        };
