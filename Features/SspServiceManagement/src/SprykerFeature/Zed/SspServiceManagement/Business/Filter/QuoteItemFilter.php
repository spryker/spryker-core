...
namespace SprykerFeature\Zed\SspServiceManagement\Business\Filter;
...
class QuoteItemFilter implements QuoteItemFilterInterface
{
     ...
     * @var string
     */
    protected const MESSAGE_INFO_SERVICE_WITHOUT_SHIPMENT_TYPE_REMOVED = 'ssp-service-management.info.service-without-shipment-type.removed';

    /**
     * @var \SprykerFeature\Zed\SspServiceManagement\SspServiceManagementConfig
     */
    protected SspServiceManagementConfig $config;
    ...
    public function __construct(
        SspServiceManagementConfig $config,
    ...
    protected function isFiltrationNeeded(ItemTransfer $itemTransfer): bool
    {
        ...
        $isServiceItem = in_array($this->config->getProductServiceTypeName(), $itemTransfer->getProductTypes(), true);
    ...
    protected function addFilterMessage(string $sku, array $messageTransfersIndexedBySku): array
    {
        ...
        $messageTransfersIndexedBySku[$sku] = (new MessageTransfer())
            ->setValue(static::MESSAGE_INFO_SERVICE_WITHOUT_SHIPMENT_TYPE_REMOVED)
            ->setParameters([
